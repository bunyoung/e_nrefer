...
IF(@$_POST['assreturn']=='1'){$assreturn='1';}else{$assreturn='0';}
$delete_date=date("Y-m-d H:i:s");
$create_user_id=@$_SESSION['name'];

$sql_add = " INSERT INTO hyass (
assname,
asstatus,
assreturn,
delete_date,
create_user_id)
VALUES ('$name','$status','$assreturn','$delete_date','$create_user_id');";
                        ...
                        <div class="col-lg-12">
                            <div class="box">
                                <header>
                                    ...
                                    <h5>เพิ่มข้อมูลประเภทของผู้ป่วยที่ขอใข้บริการ
                                    </h5>
                                </header>
                                <p>
                                <form class="form-horizontal" action="sys_admin_type_patients_add.php" method=POST
                                    target="">
                                    <div class="form-group">
                                        <label for="name" class="control-label col-lg-2">ประเภทการให้บริการ
                                        </label>
                                        <div class="col-lg-4">
                                            <input type="text" id="name" name="name" placeholder="ประเภทผการให้บริการ"
                                                class="form-control" value="" required>
                                        </div>
                                    </div>
                                    <!-- /.form-group -->
                                    <div class="form-group">
                                        <label class="control-label col-lg-2">ใช้ตอนส่งกลับ
                                        </label>
                                        <div class="col-lg-2">
                                            <div class="checkbox">
                                                <label>
                                                    <input class="status" name="assreturn" type="checkbox" value="1">
                                                </label>
                                            </div>
                                        </div>
                                    </div>
...
?>
                                        <tr>
                                            ...
                                            <td>
                                                <center>
                                                    <?php 
                                                    IF($rs_deb['assreturn']=='0'){
                                                        echo'<a href="#myModal_type_patient" data-toggle="modal" data-id="'.$rs_deb['hyass'].'" class="btn btn-success btn-xs btn-grad">';
                                                    }ELSE{
                                                        echo'<a href="#myModal_type_patient" data-toggle="modal" data-id="'.$rs_deb['hyass'].'" class="btn btn-danger btn-xs btn-grad">';}

                                                    IF($rs_deb['assreturn']=='0'){
                                                        echo '<i class="fa fa-thumbs-o-up"></i> : ไม่ได้ใช้';
                                                    }ELSE{
                                                        echo '<i class="fa fa-times"></i> : ใช้';
                                                    }
                                                    echo'</a>'; ?>
                                                </center>
                                            </td>
                                            <td>
                                                <center>
                                                    <?php IF($rs_deb['asstatus']=='0'){echo'<a href="#myModal_type_patient" data-toggle="modal" data-id="'.$rs_deb['hyass'].'" class="btn btn-success btn-xs btn-grad">';}ELSE{echo'<a href="#myModal_type_patient" data-toggle="modal" data-id="'.$rs_deb['hyass'].'" class="btn btn-danger btn-xs btn-grad">';}
IF($rs_deb['asstatus']=='0'){echo '<i class="fa fa-thumbs-o-up"></i> : Edit';}ELSE{echo '<i class="fa fa-times"></i> : Edit';}
echo'</a>'; ?>
                                                </center>
                                            </td>
                                        </tr>
...
$(document).ready(function() {
    $('#myModal_type_patient').on('show.bs.modal', function(e) {
        ...
        $.ajax({
            type: 'post',
            url: 'sys_admin_type_patients_edit.php', //Here you will fetch records
            data: {
                'hid': hid
            }, //Pass $id
