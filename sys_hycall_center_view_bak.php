...
{
    $hitem=@$_POST['hitem'];
    $sql_hycenter = "UPDATE asssent SET x1='C',x3='C'WHERE hyitem='$hitem' ";
    $result_hycent_edit = mysqli_query($conn,$sql_hycenter); mysql_error();
    echo $result_hycent_edit;
    if($result_hycent_edit== TRUE) {
...
if ($result_hycent_edit== TRUE) {
    ...
    if($token <>''){
       ...
       $str="\r\n".'งาน : งานเคลื่อนย้าย ผป '. 
            ...
            // "\r\n" .'http://192.168.99.15/hycenter/xxxx.php?id='.$hyitem;
            $res = notify_message($str,$token);
            print_r($res);
    }

    // กรณีทีไม่ม่ Line ส่งรวมกันทั้งหมด
    $token = "IbSrDmcEOjFbebDc1pOde9YUtTN7clng3Xi5nxkwqlC"; //ใส่Token ที่copy เอาไว้
    $headers    = [
            'Content-Type: application/x-www-form-urlencoded',
            'Authorization: Bearer '.$token
    ];
...
<!-- ส่วนงานของ เวชภัณฑ์และสิี่งของ กรณีที่รับงาน-->
<?PHP
if(@$_POST['REDIT']){
    $hytem=@$_POST['htem'];
    $idcard=@$_POST['idcard'];
    ...
    if(@$_POST['REDIT']){
        ...
        if ($result_hycent_edit== TRUE) 
        {
            ...
            if($token <>'')
            {
               ...
               // กรณีทีไม่ม่ Line ส่งรวมกันทั้งหมด
               $token = "hDE49DzWhKbcnRCu2yIBXIeCZifbNRo7dwI1xJahcAu"; //ใส่Token ที่copy เอาไว้
               $headers= [
                   'Content-Type: application/x-www-form-urlencoded',
                    'Authorization: Bearer '.$token
               ];
...
if(@$_POST['RFEDIT'])
{
    ...
    $sql_hycenter = "UPDATE asssent
        SET perto ='$uptime',X1='$pstatus',perremark='$perremark' WHERE hyitem='$hytem'; ";       
    $result_hycent_edit = mysqli_query($conn,$sql_hycenter); mysql_error();
    if ($result_hycent_edit== TRUE) 
    {
        $sql="SELECT * FROM v_asmonitor WHERE hyitem='$hytem' " ;
...
<body>
    ...
    <div id="content3" style="margin-top:-50px;">
        <div class="inner bg-light lter">
            <ul class="nav nav-tabs" style="color:#FF9933;font-weight: bold;">
                <p>
                    <li class="active">
                        <a data-toggle="tab" href="#home">
                            <i class="fa fa-ambulance fa-lg" aria-hidden="true" style="color:#6666FF;">
                            </i> รายการเวชภัณฑ์และสิ่งของ ประจำวันที่
            ...
            <div class="tab-content">
                ...
                <div id="menu1" class="tab-pane fade">
                    <div class="panel-group">
                        <?php include('sys_hycall_center_clean_now.php'); ?>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
$(document).ready(function() {
    $('#myModal_receive_print').on('show.bs.modal', function(e) {
        var hitem = $(e.relatedTarget).data('id');
        $.ajax({
            type: 'post',
            url: 'sys_logistic_report_001.php', //Here you will fetch records
            data: {
                'hyitems': hitem
            }, //Pass $id
            success: function(data) {
...
<div class="modal fade" id="myModal_receive_print" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;
