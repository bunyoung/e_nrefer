...
while($objResult = oci_fetch_array($objParse,OCI_BOTH))
{
    ...
    $timing_a=$objResult['TIMING_A'];
    $warning_a=$objResult['WARNING_A'];
    $warning_b=$objResult['WARNING_B'];
    $dquan=$objResult['QUANTITY'];
    echo $psrv.' aaaaaaaaaaaaaaaa';exit();
    $d_con .='
    <tr>
        ...
        <td style="border:1px solid;font-size:14px;">'.$using.'<br>'.$warning_a.'<br>'.$warning_b.'</td>
...
<table width="100%" style="border:1px solid;border-collapse: collapse;font-size:18pt;margin-top:10px;">
    ...
    while($rs = mysqli_fetch_assoc($result))
    {
        ...
        <tr>
            <td style="font-size:22px;"><b>รายการยา</b></td>
        </tr>
        <tr>
            <td>'.$d_con.'</td>
        </tr>
