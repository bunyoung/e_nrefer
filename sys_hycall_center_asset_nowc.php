...
<!--onload=”javascript:setTimeout(“location.reload(true);”,90000);”> -->
    ...
    <p>
    <table id="c_table" class="cell-border row-border compact stripe" style="width:100%;">
        <thead>
            <tr>
                <td align='center'><strong>ลำดับ</strong></td>
                <td align='center'><strong>เลขที่อ้างอิง</strong></td>
                <td><strong>ความต้องการ</strong></td>
...
#SQL
$sql ="SELECT * FROM v_asmonitor_c WHERE dgroup='C' AND x1 not in ('F','X','C') ORDER BY assname";
...
?>
            <tr style:"margin-top:0";>
                ...
                <td>
                    ...
                    if($arr['dgroup']='C'){
                        IF($arr['asscolor']=="R") {
                            echo'<a href="sys_logistic_report_003.php?id='.$arr['hyitem'].' data-toggle="modal" data-id="'.$arr['hyitem'].'"
                            class="btn btn-red btn-grad DISABLED" target="_blank">';
                        }else{
                            IF($arr['asscolor']=="G"){
                                echo'<a href="sys_logistic_report_003.php?id='.$arr['hyitem'].' data-toggle="modal" data-id="'.$arr['hyitem'].'"
                                class="btn btn-green btn-grad DISABLED" target="_blank">';
                            }else{
                                IF($arr['asscolor']=="O"){
                                    echo'<a href="sys_logistic_report_003.php?id='.$arr['hyitem'].' data-toggle="modal" data-id="'.$arr['hyitem'].'"
                                    class="btn btn-orange btn-grad DISABLED" target="_blank">';
                                }else{
                                    if($arr['asscolor']=='Y'){
                                        echo'<a href="sys_logistic_report_003.php?id='.$arr['hyitem'].' 
                                        target="_blank" data-toggle="modal" data-id="'.$arr['hyitem'].'"
                                        class="btn btn-yellow  btn-grad DISABLED" target="_blank">';   
                                    }else{
                                        echo'<a href="sys_logistic_report_003.php?id='.$arr['hyitem'].' 
                                        target="_blank" data-toggle="modal" data-id="'.$arr['hyitem'].'"
                ...
                <td style="alight:right" ;><?php echo $arr['peramt'].'  '.$arr['unit'] ?></td>
                ...
                <td><?php echo $arr['typeb'] ?></td>
                <td><?php echo $arr['typea'] ?></td>
                <td><?php echo $arr['perrend'] ?></td>
