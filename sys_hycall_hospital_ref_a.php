...
<body>
    <div class="container-fluid" style="padding: 2px 2px 2px;">
        <div class="commenta justify-content-md-center">
            <table id="dataTable-x" class="display dataTable" role="grid" 
                style="widht:100%;box-shadow: rgba(0, 0, 0, 0.19) 0px 10px 20px, rgba(0, 0, 0, 0.23) 0px 6px 6px; 
                           overflow-x: scroll; overflow-y: scroll;font-size 16px;  
                           white-space: word-wrap: break-word; cellspacing:0;">
                <thead
                    style="margin-top:0px;background-color:#214574;color:#F1F8E9">
                    <tr>
                        <td>
                            <center>No.</center>
                        </td>
                ...
                $sql="SELECT * FROM v_rf_detail  WHERE rf_hos_send_to = '$hcode' AND end_refer_end='N'  AND 
                            rf_no_thairefer <> '' AND rf_ptype_expire<>'' AND rf_place_money<>'' AND rf_pttype<>''
                            ORDER BY substr(rf_date,7,4), substr(rf_date,4,2) DESC,substr(rf_date,1,2) DESC,rf_hos_send_to ASC";
				...
				While($rs=mysqli_fetch_array($query)) {
                    ...
                    if($rs['hosp_recive_status']=='Y'){
                        echo '<tr style="font-size:17px;color:#034638"> ';
                    }else{
